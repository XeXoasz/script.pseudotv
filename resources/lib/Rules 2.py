...
class HandleChannelPosition(BaseRule):
    def __init__(self):
        self.name = [LANGUAGE(30117)] 
        self.optionLabels = [LANGUAGE(30117)] 
        self.optionValues = ["upper left 16:9",]
        self.myId = 18
        ...
        self.selectBoxOptions = ["upper left 16:9", "upper right 16:9", "lower right 16:9", "lower left 16:9", "upper left 4:3", "upper right 4:3", "lower right 4:3", "lower left 4:3"]

    def copy(self):
        copy = HandleChannelPosition()
        return copy

    def getTitle(self):
        if self.optionValues[0] == "upper left 16:9":
            return LANGUAGE(30117)
        elif self.optionValues[1] == "upper right 16:9":
            return LANGUAGE(30118)
        elif self.optionValues[2] == "lower right 16:9":
            return LANGUAGE(30119)
        elif self.optionValues[3] == "lower left 16:9":
            return LANGUAGE(30120)
        elif self.optionValues[4] == "upper left 4:3":
            return LANGUAGE(30231)
        elif self.optionValues[5] == "upper right 4:3":
            return LANGUAGE(30232)
        elif self.optionValues[6] == "lower right 4:3":
            return LANGUAGE(30233)
        elif self.optionValues[7] == "lower left 4:3":
    ...
    def runAction(self, actionid, overlay, channeldata):
        if actionid == RULES_ACTION_OVERLAY_SET_CHANNELBUG:
            ...
            if self.optionValues[0] == "upper left 16:9":
               overlay.channelBugPosition = self.channelBugRulesPosition["upper left 16:9"]
               self.log("setting channel position to upper left 16:9")
            elif self.optionValues[1] == "upper right 16:9":
                 overlay.channelBugPosition = self.channelBugRulesPosition["upper right 16:9"]
                 self.log("setting channel position to upper right 16:9")
            elif self.optionValues[2] == "lower right 16:9":
                 overlay.channelBugPosition = self.channelBugRulesPosition["lower right 16:9"]
                 self.log("setting channel position to lower right 16:9")
            elif self.optionValues[3] == "lower left 16:9":
                 overlay.channelBugPosition = self.channelBugRulesPosition["lower left 16:9"]
                 self.log("setting channel position to lower left 16:9")
            elif self.optionValues[4] == "upper left 4:3":
                 overlay.channelBugPosition = self.channelBugRulesPosition["upper left 4:3"]
                 self.log("setting channel position to upper left 4:3")
            elif self.optionValues[5] == "upper right 4:3":
                 overlay.channelBugPosition = self.channelBugRulesPosition["upper right 4:3"]
                 self.log("setting channel position to upper right 4:3")
            elif self.optionValues[6] == "lower right 4:3":
                 overlay.channelBugPosition = self.channelBugRulesPosition["lower right 4:3"]
                 self.log("setting channel position to lower right 4:3")
            elif self.optionValues[7] == "lower left 4:3":
                 overlay.channelBugPosition = self.channelBugRulesPosition["lower left 4:3"]
                 self.log("setting channel position to lower left 4:3")

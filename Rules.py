...
class HandleChannelPosition(BaseRule):
    def __init__(self):
        ...
        self.optionValues = ["upper left 16:9", "upper right 16:9", "lower right 16:9", "lower left 16:9", "upper left 4:3", "upper right 4:3", "lower right 4:3", "lower left 4:3"]
        self.myId = 18
    ...
    def getTitle(self):
        selected_option = self.optionValues[0]
        return LANGUAGE(CHANNELBUG_POS_MAPPING[selected_option][0])
    ...
    def runAction(self, actionid, overlay, channeldata):
        if actionid == RULES_ACTION_OVERLAY_SET_CHANNELBUG:
            ...
            selected_option = self.optionValues[0]
            overlay.channelBugPosition = CHANNELBUG_POS_MAPPING[selected_option][1]
            self.log("setting channel position to {}".format(selected_option))
